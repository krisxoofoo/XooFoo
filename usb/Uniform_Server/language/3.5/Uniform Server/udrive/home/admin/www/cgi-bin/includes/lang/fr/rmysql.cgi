...
if ($run) {
  	if ($ENV{'QUERY_STRING'}eq"F"){
		...
		print <<ENDDD;
		Le serveur MySQL &eacute;tait en fonctionnement.
		<br />
		Mais si vous pensez que ceci est &eacute;trange, cliquez sur ce <a href="$ENV{SCRIPT_NAME}?F">lien</a>
		<br />
		</p>
 		</div>
ENDDD
  	...
  	print <<ENDDD;
  	Le serveur MySQL est d&eacute;marr&eacute;.
  	</p>
  	</div>
ENDDD
  	require footer;
