...
if($mod_dbi!=1){
	...
	if ($dbi_mysql_exists !=1 ) { print $err."<br>Critical error: modul DBD::mysql not found! crondump.pl can't establish a connection to the MySQL-Database if this modul isn't installed! Please install DBI::mysql!</font>"; }
...
if($mod_auth!=1){
    ...
    print $ok."Found modul Authen::SASL. OK. crondump.pl can use SMTP_AUTH.</font><br>\n";
}
...
if($mod_base64!=1){
    ...
    print $ok."Found modul MIME::Base64. OK. crondump.pl can encode user/pass for SMTP_AUTH.</font><br>\n";
}
...
if($mod_ssl !=1){
    ...
    print $ok."Found modul Net::SMTP::SSL. OK. crondump.pl can send emails with ssl.</font><br>\n";
}
