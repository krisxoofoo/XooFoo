...
# 17-12-2009  Added Opera USB support
#############################################################################
# === Apache ===
...
$usr_profile="$base_f/apps/opera/profile/operaprefs.ini"; 	// Path to Opera 
$pac_path = "\"file:///$base_f/pac/my_hosts.pac\"";          // Path to pac file
...
function start_opera($page){
  ...
  // Global variables for paths and files
   global $base_f; global $usf_uniserv;
  ...
  $cmd1 = "start ";
  $cmd2 = "$usf_uniserv \"";
  $cmd3 = "$base_f/apps/opera/OperaUSB.exe -new-tab $page\" ";
  $cmd = $cmd1.$cmd2.$cmd3;
...
function get_opera_exe(){
  ...
  while($file=readdir($dir_handle)){              // Loop through folder names 
    if(preg_match("/^Opera_USB.+exe/", $file)){ // Search for executable name 
...
function stop_opera(){
  ...
  $cmd = "$usf_pskill $opera_exe c"; // c=kill command 
  exec($cmd,$dummy,$return);           // run command 
}
//================================================= END Stop Opera browser ===
