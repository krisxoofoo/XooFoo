...
# 1-3-2010 Updated for Opera Portable support
###############################################################################
*/
...
print " #--------------------------------------------------------------------------------------------#\n\n";
 ...
 // Portable Opera
if(file_exists(USF_HOSTS_PAC)){ // Does PAC file exists use  portable Opera
 $opera_name_old = get_opera_exe();               # Opera executable name
                                                      # New Opera executable name
 if(preg_match("/([a-zA-Z\_]+)(\d*)(\.exe)/", $opera_name_old, $matches)){  
   $opera_name=  $matches[1].($matches[2]+1).$matches[3];                                     
 }
}                    
...
$opera_name    = prompt_user(" Current Opera name   = $opera_name_old  Proposed name ", $opera_name);
}
...
//=============== UPDATE OPERA ==========================
if(file_exists(USF_HOSTS_PAC)){ // Does PAC file exists use  portable Opera
  $s_str = $opera_name_old;
  $s_str = preg_quote($s_str,'/');        // Convert to regex format
  $s_str = '/'.$s_str.'/';                // Create regex pattern
  file_search_replace(US_BASE_F."/apps/opera/operaprefs_default.ini",$s_str,$opera_name);

 // Rename Firefox exe

 $old = US_BASE_F."/apps/opera/$opera_name_old";
 $new = US_BASE_F."/apps/opera/$opera_name";
