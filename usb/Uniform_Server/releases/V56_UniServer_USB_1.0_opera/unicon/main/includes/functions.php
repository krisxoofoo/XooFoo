...
# 16-2-2010  Added opera portable support
#############################################################################
# === Apache ===
...
#
# === opera Portable
# get_opera_exe()                    - Returns opera executable file name
# start_opera()                      - Start opera browser
# stop_opera()                       - Stop opera browser
# function  update_user_pref_opera() - Update user file with new PAC path
#----------------------------------------------------------------------------
*/
...
// $cmd = US_MYSQL_BIN."/$MySQL_exe --install ".USC_MYSQL_SERVICE_NAME." --defaults-file=USF_MYSQL_INI";
 $cmd = US_MYSQL_BIN."/$MySQL_exe --install ".USC_MYSQL_SERVICE_NAME."\"";
 exec($cmd,$dummy,$return);      // Install service 
...
function run_location_tracker(){
  ...
  if(get_location_tracker()!= US_BASE_F){   // Server has been moved
    ...
    file_search_replace(USF_PHP_INI_DEV,$s_str,US_BASE_F);  // Update PHP delvelopment ini
...
//=== Start opera ===========================================================
function start_opera($page){
  // Note page has the following format
  // http://localhost:port/path/page

  stop_opera();                // Kill opera if running  
  update_user_pref_opera();    // Update pref file with path
...
function get_opera_exe(){
  ...
  $opera_folder = US_BASE_F."/apps/opera/"; // opera folder
  ...
  closedir($dir_handle);
  return $opera_exe;                          // Return executable Apache*.exe
}
//=============================================== END Get opera executable ===


//=== Stop opera browser ====================================================
function stop_opera(){

  $opera_exe = get_opera_exe();        // get program name   
  $cmd = USF_PSKILL_EXE." $opera_exe c"; // c=kill command 
  exec($cmd,$dummy,$return);               // run command 
}
//================================================= END Stopopera browser ===

// === Update user file with new PAC path =====================================
// Has this format: user_pref("network.proxy.autoconfig_url", "absolute path");
function  update_user_pref_opera(){

$usr_profile=US_BASE_F."/apps/opera/profile/operaprefs.ini"; // Path to opera 
