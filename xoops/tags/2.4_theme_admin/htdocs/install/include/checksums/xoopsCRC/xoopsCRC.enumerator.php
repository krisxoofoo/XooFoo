...
if (!class_exists('xoopsCRC_enumerator'))
{
	
	
	class xoopsCRC_enumerator extends xoopsCRC
	{
		...
		private function setElements($base)
		{
			...
			foreach ($base->base as $key => $data)
			{
				if (strlen((string)$data)==1)
				{
					...
					{
						$offset = array("xod" => (int)substr(decbin(ord($data)),3,1),
										"xoa" => (int)substr(decbin(ord($data)),2,1),
										"xoe" => (int)substr(decbin(ord($data)),5,1),
										"xoc" => (int)substr(decbin(ord($data)),1,1),
										"xob" => (int)substr(decbin(ord($data)),2,1));							
						if (substr(decbin(ord($data)),4,1)==1)
						{
							$offset['xof'] = 1;
						} else {
							$offset['xof'] = 0;
					...
					{
						$offset = array("xof" => (int)substr(decbin(ord($data)),4,1),
										"xob" => (int)substr(decbin(ord($data)),5,1),
										"xoa" => (int)substr(decbin(ord($data)),6,1),
										"xoc" => (int)substr(decbin(ord($data)),2,1),
										"xod" => (int)substr(decbin(ord($data)),1,1),
										"xoe" => (int)substr(decbin(ord($data)),3,1));				
					} elseif (strlen(decbin(ord($data)))==7) 
					{
						$offset = array("xoc" => (int)substr(decbin(ord($data)),1,1),
										"xob" => (int)substr(decbin(ord($data)),5,1),
										"xof" => (int)substr(decbin(ord($data)),6,1),
										"xod" => (int)substr(decbin(ord($data)),2,1),
										"xoa" => (int)substr(decbin(ord($data)),3,1),
										"xoe" => (int)substr(decbin(ord($data)),4,1));
					}			
				} else {
					$offset = array("xob" => (int)substr(decbin(ord(substr($key,strlen($key)-1,1))),5,1),
									"xoe" => (int)substr(decbin(ord(substr($key,strlen($key)-1,1))),6,1),
									"xod" => (int)substr(decbin(ord(substr($key,strlen($key)-1,1))),4,1),
									"xoc" => (int)substr(decbin(ord(substr($key,strlen($key)-1,1))),3,1),
									"xoa" => (int)substr(decbin(ord(substr($key,strlen($key)-1,1))),5,1),
									"xoc" => (int)substr(decbin(ord(substr($key,strlen($key)-1,1))),1,1));
