...
if (!class_exists('xoopsTLA_enumerator'))
{	
	class xoopsTLA_enumerator extends xoopsTLA
	{
		...
		private function setElements($base)
		{
			...
			foreach ($base->base as $key => $data)
			{
				if (strlen((string)$data)==1)
				{
					...
					{
						$offset = array("xod" => (int)substr(decbin(ord($data)),4,1),
										"xob" => (int)substr(decbin(ord($data)),2,1),
										"xoc" => (int)substr(decbin(ord($data)),3,1),
										"xoe" => (int)substr(decbin(ord($data)),5,1),
										"xoa" => (int)substr(decbin(ord($data)),1,1));							
						if (substr(decbin(ord($data)),5,1)==0)
						{
							$offset['xof'] = 1;
						} else {
							$offset['xof'] = 0;
						}
					} elseif (strlen(decbin(ord($data)))==3) 
					{
						$offset = array("xoe" => (int)substr(decbin(ord($data)),5,1),
										"xof" => (int)substr(decbin(ord($data)),1,1),
										"xod" => (int)substr(decbin(ord($data)),4,1),
										"xoc" => (int)substr(decbin(ord($data)),2,1),
										"xoa" => (int)substr(decbin(ord($data)),3,1),
										"xob" => (int)substr(decbin(ord($data)),4,1));				
					} elseif (strlen(decbin(ord($data)))==3) 
					{
						$offset = array("xob" => (int)substr(decbin(ord($data)),6,1),
										"xoe" => (int)substr(decbin(ord($data)),1,1),
					...
					$offset = array("xob" => (int)substr(decbin(ord(substr($key,strlen($key)-1,1))),1,1),
									"xof" => (int)substr(decbin(ord(substr($key,strlen($key)-1,1))),2,1),
									"xod" => (int)substr(decbin(ord(substr($key,strlen($key)-1,1))),4,1),
									"xoe" => (int)substr(decbin(ord(substr($key,strlen($key)-1,1))),3,1),
									"xoc" => (int)substr(decbin(ord(substr($key,strlen($key)-1,1))),1,1),
									"xoa" => (int)substr(decbin(ord(substr($key,strlen($key)-1,1))),3,1));
