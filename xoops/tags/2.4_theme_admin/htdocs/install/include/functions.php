...
/**
 * *#@+
 * Xoops Return CRC Class System Key
 */
...
function xoBuildLicenceKey()
{
    ...
    srand((((float)('0' . substr(microtime(), strpos(microtime(), ' ') + 1, strlen(microtime()) - strpos(microtime(), ' ') + 1))) * mt_rand(30, 99999)));
    $checksums = array(1 => 'xoopsCRC', 2 => 'xoopsFLA', 3 => 'xoopsTLA');
    $type = rand(1, 3);
    error_reporting(E_ERROR);
    foreach ($_SERVER as $key => $data) {
        if ($xoopskey = XoopsCRC(serialize($data), 0, 0, $checksums[$type])) {
    ...
    if ($xoopskey = XoopsCRC(__FILE__, 0, 0, $checksums[$type])) {
        $xoops_serdat['file'] = $xoopskey['key'];
    }
    if ($xoopskey = XoopsCRC(basename(__FILE__), 0, 0, $checksums[$type])) {
        $xoops_serdat['basename'] = $xoopskey['key'];
    }
    if ($xoopskey = XoopsCRC(dirname(__FILE__), 0, 0, $checksums[$type])) {
        $xoops_serdat['path'] = $xoopskey['key'];
    }
    if ($xoopskey = XoopsCRC(microtime(), 0, 0, $checksums[$type])) {
        $xoops_serdat['time'] = $xoopskey['key'];
    }
    if ($xoopskey = XoopsCRC(XOOPS_VERSION, 0, 0, $checksums[$type])) {
        $xoops_serdat['basename'] = $xoopskey['key'];
    }
    if ($xoopskey = XoopsCRC(XOOPS_LICENSE_CODE, 0, 0, $checksums[$type])) {
        $xoops_serdat['path'] = $xoopskey['key'];
    }
    if ($xoopskey = XoopsCRC(XOOPS_LICENSE_TEXT, 0, 0, $checksums[$type])) {
        $xoops_serdat['time'] = $xoopskey['key'];
    }
    foreach ($xoops_serdat as $key => $data){
        $lpop = rand(3, rand(5, 30));
        $xoops_key .= substr($data, rand(0, strlen($data) - $lpop), $lpop);
    }
    while (strlen($xoops_key) > 40) {
        $lpos = rand(5, strlen($xoops_key));
